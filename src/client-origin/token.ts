...
import { sendAndConfirmTransaction } from './util/send-and-confirm-transaction';
 const  ORIGINE_PROGRAMM_ID = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"
   export const  ASSOCIATED_TOKEN_PROGRAM_ID: PublicKey = new PublicKey(
    'ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL'
  );
   const  ADAPTABLE_PROGRAMM_ID = "CxUSh8EJmP7q5MWf94sTZNThAUxMHU9Y37YqUcGRCZca"
...
/**
 ...
 static async createMintReturnInstruction(
  ...
  let instruction2=TokenOrigin.createInitMintInstruction(
    programId,
    mintAccount.publicKey,
    decimals,
    mintAuthority,
...
/**
   ...
   * @return Public key of the new empty account
   */
 async createAccountwithPartialsign(owner: PublicKey): Promise<any> {
  const balanceNeeded = await TokenOrigin.getMinBalanceRentForExemptAccount(
    this.connection,
  );
  const newAccount = new Account();
  ...
  */
   ...
   */
   async createAccountOfInsctruction(owner: PublicKey): Promise<any> {
    ...
    const mintPublicKey = this.publicKey;
    let instruction2=TokenOrigin.createInitAccountInstruction(
      this.programId,
      mintPublicKey,
      newAccount.publicKey,
      owner,
   ...
   */
    async mintToReturnInstruction(
      ...
      let instruction= TokenOrigin.createMintToInstruction(
        this.programId,
        this.publicKey,
        dest,
        ownerPublicKey,
  ...
  async createAssociatedTokenAccount(owner: PublicKey): Promise<PublicKey> {
   ...
   // let token=new Token(this.connection,this.publicKey,this.programId,)
console.log("associatedAddress associatedAddress ",associatedAddress.toBase58())
let info=await this.connection.getAccountInfo(associatedAddress)
 ...
 */
  ...
  async createAssocitedTokenAccountOfInsctruction(
    owner: PublicKey,
    associatedAddress: PublicKey,
  ): Promise<PublicKey> {
 ...
 */
 ...
 async transfer(
  ...
  async transferwithoutPartialSigner(
    source: PublicKey,
    destination: PublicKey,
    owner: any,
    multiSigners: Array<Account>,
...
/*
   ...
   */
   async approveInstruction(
    ...
    let instruction=TokenOrigin.createApproveInstruction(
      this.programId,
      account,
      delegate,
      ownerPublicKey,
